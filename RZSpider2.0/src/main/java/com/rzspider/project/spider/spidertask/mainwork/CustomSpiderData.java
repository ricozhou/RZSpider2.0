...
public class CustomSpiderData {
	...
	public XSSFWorkbook analyzeCustomSpiderData(StartSpiderInfo startSpiderInfo, List<Spiderdata> list) {
		if (list == null || list.size() < 1) {
			return null;
		}
		// 思路是获取每条json键值对的数量，将key和value分别加入数组，list第一个值是key，其它是是value
		// 自定义爬虫数据，直接将json的value取出成list<String[]>
		List<String[]> stringArrayList = CustomSpiderDataUtils.jsonStringListToStringArrayList(list);
