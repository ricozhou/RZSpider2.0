...
public class LMNSpiderTaskService {
	// private LMNSpiderTaskService lMNSpiderTaskService;

	// 爬取页数
	// @Override
	public int[] getPageAndNum() throws FailingHttpStatusCodeException, MalformedURLException, IOException {
		...
		// if (request.getUrl().toExternalForm().contains("Calendar.js")) {//
		// bfdata.js是需要修改编码的js，你可以修改该处逻辑
		// String content = response.getContentAsString("UTF-8");
		// System.out.println(content.indexOf("function document.onclick()"));
		// String content2=content.replace("function document.onclick()",
		// "document.onclick()=function()");
		// String content3=content2.replace("function
		// String.prototype.trim(){return
		// this.replace(/(^\\s*)|(\\s*$)/g,\"\");}",
		// "String.prototype.trim=function(){return
		// this.replace(/(^\\s*)|(\\s*$)/g,\"\");}");
		// System.out.println(content3);
		// WebResponseData data = new
		// WebResponseData(content3.getBytes("UTF-8"), response.getStatusCode(),
	...
	// 获取详细信息
	// @Override
	public void getLMNLandDetail(LMNLandParams lMNLandParams, List<LMNLandMessageDetail> lMNLandMessageDetailList,
		...
		// page);
		boolean isSuccess = setLMNLandMeg(lMNLandMessageDetailList, page);
	}

	// 取数据
	// @Override
	public boolean setLMNLandMeg(List<LMNLandMessageDetail> lMNLandMessageDetailList, HtmlPage page)
		...
		for (int i = 1; i < n; i++) {
			...
			for (int j = 1; j < n2; j++) {
				...
				getLMNLandDetail(lMNLandMessageDetailList, pg2, notice, noticeLink);
			}
		}
		return true;
	}
	...
	// 存入到对象中
	// @Override
	public void getLMNLandDetail(List<LMNLandMessageDetail> lMNLandMessageDetailList, HtmlPage pg, String notice,
			String noticeLink) {
		// 验证状态
		// if (!JSLandMainGUI.JSLandFlag) {
		// return;
