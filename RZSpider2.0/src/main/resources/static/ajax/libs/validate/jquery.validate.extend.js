...
$(document).ready(function(){
	...
	jQuery.validator.addMethod("english",function(value,element){
		var length = value.length;
		var chrnum = /^([a-zA-Z]+)$/;
	    return this.optional(element) || (length<10&&chrnum.test(value));
	},"只能是长度小于10的英文字母");
	//不能带空格
	jQuery.validator.addMethod("isBlank",function(value,element){
		var length = value.length;
		return ((length==0)||($.trim(value) != '')&&(value.indexOf(" ")==-1));
	},"不能带空格");
	
	//文章内容不能为空
	jQuery.validator.addMethod("cannotNUll",function(value,element){
		value=	$("#content_sn").summernote('code');
		var length = value.length;
		return this.optional(element)||(length >0 && length <20000);
	},"文章不能为空且字数不超过两万");
	
	//验证两个数的大小
	jQuery.validator.addMethod("checkStartEndPage",function(value,element){
		var returnVal = false;
	...
	jQuery.validator.addMethod("checkDateTime",function(value,element){
		...
		return this.optional(element)||(length == 10 && dateTime.test(value));
	},"时间格式yyyy-MM-dd");
	...
	//验证日期大小
	jQuery.validator.addMethod("checkDateSize",function(value,element){
		var returnVal = false;
		var startDateTime = $("#startDateTime").val().replace(/-/g,'');
		var endDateTime = $("#endDateTime").val().replace(/-/g,'');
