...
function initConsoleEditor() {
	...
	consoleEditor = CodeMirror
			.fromTextArea(document.getElementById("console"),
					{
						// mode
						// :
						// "text/groovy",
...
function showRMenu(type, x, y, treeNode) {
	...
	if (treeNode.name == 'public_node_moudules') {
		// 可上传，下载
		...
		$("#deleteFile").css('display', 'block');
			...
			&& treeNode.getParentNode().name() == 'public_node_moudules'
			&& treeNode.getParentNode().getParentNode().getParentNode() == null) {
		$("#importFile").css('display', 'none');
		$("#newFile").css('display', 'none');
		$("#openFile").css('display', 'none');
...
function deleteFile() {
	...
	// 删除文件
	// 正在运行则不能删除
	var runflag = $("#run").css("display") === 'inline';
	if (runflag) {
